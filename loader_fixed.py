...
def abrir_site_renovacao():
    url_renovacao = "https://www.fovdark.shop/pricing"
...
class App(ctk.CTk):
    ...
    def login(self):
        ...
        try:
            # Fixed: Use correct login endpoint with JSON data
            headers = {"Content-Type": "application/json"}
            login_data = {"email": email, "password": senha}
            
            r = requests.post(f"{API_URL}/api/auth/login", json=login_data, headers=headers)
            ...
            # Create session cookies from login response
            session_cookies = r.cookies
            
            # Check license status using dashboard endpoint
            r2 = requests.get(f"{API_URL}/api/dashboard", cookies=session_cookies)
            if r2.status_code != 200:
                messagebox.showerror("Erro", f"Erro ao checar licença.\nStatus: {r2.status_code}")
                return

            dados_dashboard = r2.json()
            license_data = dados_dashboard.get("license")
            ...
            # Update license HWID using correct endpoint
            hwid_data = {"hwid": hwid_atual}
            res_hwid = requests.post(f"{API_URL}/api/licenses/update-hwid", 
                                   json=hwid_data, 
            ...
            dias = license_data.get("daysRemaining", 0)
            horas = license_data.get("hoursRemaining", 0) 
            minutos = license_data.get("minutesRemaining", 0)
            
            if dias > 0:
                tempo_msg = f"{dias} dias"
            elif horas > 0:
                tempo_msg = f"{horas} horas e {minutos} minutos"
            else:
                tempo_msg = f"{minutos} minutos"
            
            messagebox.showinfo("Sucesso", f"Licença ativa! Tempo restante: {tempo_msg}")
