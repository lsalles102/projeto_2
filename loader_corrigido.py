...
# CORREÇÃO 1: URL correta (remover www)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.token = None  # CORREÇÃO: armazenar token
    ...
    def login(self):
        ...
        try:
            # CORREÇÃO 2: Usar JSON em vez de form data e endpoint correto
            r = requests.post(
                f"{API_URL}/api/auth/login", 
                json={"email": email, "password": senha},  # JSON em vez de data
            ...
            # CORREÇÃO 3: Token está em 'token' não 'access_token'
            response_data = r.json()
            self.token = response_data["token"]
            
            # CORREÇÃO 4: Headers de autorização corretos
            headers = {"Authorization": f"Bearer {self.token}"}
            hwid_atual = gerar_hwid()

            # CORREÇÃO 5: Endpoint correto para verificar licença
            ...
            # CORREÇÃO 6: Endpoint e payload corretos para salvar HWID
            res_hwid = requests.post(
                f"{API_URL}/api/hwid/save", 
                json={"hwid": hwid_atual}, 
                headers=headers
            )
            
            if res_hwid.status_code != 200:
                error_msg = "Acesso Não Autorizado. HWID não autorizado ou licença inativa."
                try:
                    error_data = res_hwid.json()
                    error_msg = error_data.get("message", error_msg)
                except:
                    pass
                ...
                messagebox.showerror("Erro", f"🚨 {error_msg}\n\n"
                                             "Isso pode indicar:\n"
                                             "• Tentativa de uso em PC não autorizado\n"
                                             "• Licença expirada ou inativa\n"
                                             "• Compartilhamento indevido de conta")
                return

            salvar_hwid_no_registro(hwid_atual)

            # CORREÇÃO 7: Campo correto para dias restantes
            ...
            messagebox.showerror("Erro", "Timeout na conexão.\nTente novamente.")
        except Exception as e:
