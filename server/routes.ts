...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // PIX Payment creation (endpoint alternativo para compatibilidade)
  app.post("/api/payments/pix/create", isAuthenticated, rateLimit(5, 60 * 1000), async (req, res) => {
    try {
      ...
      // Save payment to database with PIX information
      const payment = await storage.createPayment({
        userId: user.id,
        preferenceId: pixPayment.preferenceId,
        externalReference: pixPayment.externalReference,
        status: "pending",
        transactionAmount: pixPayment.transactionAmount, // Already in cents
        ...
        payerLastName: requestData.payerLastName,
        paymentMethodId: "pix",
        notificationUrl: `${process.env.NODE_ENV === 'production' ? 'https://fovdark.shop' : 'http://localhost:5000'}/api/payments/webhook`,
